...
class CSSSyntaxError(PremailerError):
    def __init__(self, message):
        super(PremailerError, self).__init__(message)

        # ERROR: "background-color: lighblue;"
        # Should be: "ERROR Property: Invalid value for property:
        # background-color: lighblue  - Line: 8, Column: 17"
        if message.startswith("ERROR Property: Invalid"):
            try:
                ...
                message = "{0}{1}: \"{2}: {3}\" - Line: {4} Column: {5}"\
                    .format(msg1, msg2.strip(), propertyvalue.strip(),
                            msg3.strip(), line, column)
            except:
                Exception.__init__(self, message)

        # ERROR: "background: light!blue;"
        # Should be: "ERROR Property: No CSS priority value -
        # Line: 10, Column: 17"
            ...
            try:
                ...
                message = "{0}:{1} - Line: {2}, Column: {3}"\
                    .format(message, info, line, column)
            except:
                Exception.__init__(self, message)

        # ERROR: "background #fffef0;"
        # Should be: "ERROR Property: No ":" after name found:
        # "background #ffffe0" - Line 4 Column: 28"
            ...
            try:
                ...
                message = "{0}found: \"{1}\" - Line {2} Column: {3}"\
                    .format(message, propertyvalue.strip(), line, column)
            except:
                Exception.__init__(self, message)

        # ERROR: "background: ;"
        # ERROR: "background: }#fffef0;"
        # Should be: "ERROR CSSStyleDeclaration:
        # Syntax Error in Property: background: "
        ...
        # Should be: "ERROR PropertyValue: Syntax Error in
        # Property: background: #ffef0 - Line: 4, Column: 29"
            ...
            try:
                ...
                message = "{0} :{1} : \"{2}\" - Line:{3}, Column:{4}"\
                    .format(errortype, errorlocation,
                            propertyvalue.strip(), line, column)
                
        # ERROR: "background: ##fffef0;"
        # Should be: "ERROR PropertyValue : Syntax Error in
        # Property : "background: ##fffef0" - Line: 4, Column: 29"
            except:
                # ERROR: "background:{ #fffef0;"
                ...
                if "ERROR CSSMediaRule:" in message:
                    try:
                        ...
                        message = "Found: '{0}' - Line:{1}, Column:{2}"\
                            .format(problem, line, column)
                    except:
                        Exception.__init__(self, message)
                else:
                    try:
                        ...
                        message = "We have found an error above:{0}"\
                            .format(message)

        # ** @ Media Specific Errors ** #

        # ERROR: "Gibberish-Before @media screen {"
        # Should be: "ERROR SelectorList: "@media"
        # Invalid Selector - Line: 5, Column: 9"
            ...
            try:
                ...
                try:
                    # ERROR: "@font-face a{"
                    # Should be: "ERROR Unexpected token :
                    # Found: a - Line: 14, Column: 20"
                    if message.startswith("ERROR Unexpected token (IDENT"):
                        ...
                        message = "{0}: Found: \"{1}\" - Line:{2}, Column:{3}"\
                            .format(errortype, identifiererrror, line, column)
                except:
                    Exception.__init__(self, message)

        # ERROR: "[@media screen {"
        # Should be: "ERROR Selector: "@media"
        # Unexpected CHAR. [4 - Line: 4, Column: 10"
            ...
            try:
                ...
                errortype, propertyvalue, restofmsg = info.split(':', 2)
                message = "{0}: \"{1}\" {2} - Line:{3}, Column:{4}"\
                    .format(errortype.strip(), errorrule.strip(),
                            propertyvalue.strip(), line, column)
            except:
                Exception.__init__(self, message)

        # ERROR: "@media screnen {"
        # Should be: "ERROR MediaQuery: Found: 'screnen' -
        # Line: 2, Column: 16"
        ...
        # Should do: "WARNING Property: Unknown Property name:
        # "backgrund" - Line: 4, Column: 17"
        elif message.startswith("WARNING Property:") or \
                message.startswith("WARNING CSSStylesheet: "
                                   "Unknown @rule found."):
            try:
                message, info = message.rsplit(']', 1)[0].rsplit('[', 1)
...
class HTMLElementError(PremailerError):
    def __init__(self, message):
        super(PremailerError, self).__init__(message)
        # ERROR: "<head>>"
        # Should be: "Unexpected end tag: "head" -
        # Line: 73, Column: 16 - Check Opening " head" tag."
        if str(message).startswith("Unexpected end tag"):
            try:
                ...
                message = msg.strip() + ": \"" + element.strip() + \
